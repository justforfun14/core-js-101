 ...
 *   'ABABAB','BA' => 'ABAB'
 */
function removeFirstOccurrences(str, strToRemove) {
  const index = str.indexOf(strToRemove);
  const newStr = str.split('');
  newStr.splice(index, strToRemove.length);
  return newStr.join('');
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((item) => ROT[ALPHABET.indexOf(item)])
    .join('');
}
